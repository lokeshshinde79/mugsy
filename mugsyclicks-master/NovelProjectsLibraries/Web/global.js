/*
* MaxLength Limit on Multiline Textbox
* @@params (textbox control, key event, max length, use label)
*/
function isMaxLength(obj, evt, limit, lblId) {
    var key;
    var lbl;
    var count;

    key = (typeof evt.which != "undefined") ? evt.which : evt.keyCode;
    lbl = (lblId != null) ? document.getElementById(lblId) : null;
    count = obj.value.length + 1;

    if (key == 8) { count = obj.value.length - 1 }
    if (key == 13) { count = obj.value.length }

    if (count > limit) {
        obj.value = obj.value.substring(0, limit);
        if (lbl != null) lbl.style.color = '#ff0000';
    }
    else {
        if (lbl != null) {
            ...
            if (count != -1) {
                lbl.innerHTML = count.toString() + "/" + limit.toString();
            }
            else
                lbl.innerHTML = 0 + "/" + limit.toString();
        }
    }
}
...
*  Global npJavascript
*/
function doPostBack(event, BtnId) {
    if (event.keyCode == 13) {
        __doPostBack(BtnId, '');
        return false;
    }
...
function ShowButtonAsProcessing(btn, ValidationGroup) {
...
function ShowButtonAsProcessingWithText(btn, text) {
    if (Page_ClientValidate() == false) {
        return false;
    }
...
function ShowButtonAsProcessingWithText(btn, ValidationGroup, text) {
    if (Page_ClientValidate(ValidationGroup) == false) {
        return false;
    }
